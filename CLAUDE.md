...
GitCode Insight 是一个命令行工具，用于从 GitCode 平台获取数据并生成分析报告。支持社区洞察、Issue 洞察、PR 洞察、仓库统计（含订阅用户和编程语言）和仓库综合报告。
...
├── repo_stats.py   # GitCodeRepoStats 类，仓库统计（下载/Fork/订阅用户/编程语言）
