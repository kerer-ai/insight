...
GitCode Insight 是一个命令行工具，用于从 GitCode 平台获取数据并生成分析报告。支持社区统计分析、Issue 洞察、PR 洞察、仓库统计、订阅用户统计、编程语言统计和仓库综合报告，生成可视化看板和 Markdown 报告。
...
- 下载统计：时间范围下载量、历史累计、日均下载、峰值日
- Fork 统计：Fork 总数、新增 Fork、最新 Fork 信息
...
- **智能数据采集**：自动检测数据文件，不存在则自动调用对应命令采集
...
```bash
gc-insight subscribers --repo your-repo --token your_token --days 30
```

### 7. 运行编程语言统计

```bash
gc-insight languages --repo your-repo --token your_token
```
...
| 效率 | 合并耗时 | PR 创建到合并的时间 |
| 效率 | 平均打开天数 | 当前打开 PR 的平均存活时间 |
| 效率 | 24h评审率 | 24小时内获得评审的 PR 占比 |
| 质量 | 合并率 | 已合并 PR 占总数比例 |
| 质量 | 草稿率 | 草稿 PR 占比 |
| 质量 | 冲突率 | 存在冲突的 PR 占比 |
| 质量 | CI成功率 | CI 构建成功的 PR 占比 |
...
### gc-insight subscribers

获取仓库订阅用户统计。

```bash
gc-insight subscribers --repo REPO --token TOKEN [选项]
```
...
```bash
# 获取订阅用户统计
gc-insight subscribers --repo kernel --token gct_xxxx --owner openeuler --days 30
```
...
数据已保存到: output/subscribers_openeuler_kernel_30d.json
============================================================
```

### gc-insight languages

获取仓库编程语言占比统计。

```bash
gc-insight languages --repo REPO --token TOKEN [选项]
```
...
```bash
# 获取编程语言统计
gc-insight languages --repo kernel --token gct_xxxx --owner openeuler
```
...
数据已保存到: output/languages_openeuler_kernel.json
============================================================
```
...
开始采集数据...
发现现有 Issue 数据: output/issue_insight_kernel_3d.json
发现现有 PR 数据: output/pr_insight_kernel_3d.json
未找到仓库统计数据，开始采集...
...
...
| 文件名 | 格式 | 说明 |
|--------|------|------|
| `prs_{repo}_{days}d.csv` | CSV | PR 原始数据 |
| `pr_insight_{repo}_{days}d.json` | JSON | 洞察结论数据 |
...
| 文件名 | 格式 | 说明 |
|--------|------|------|
| `subscribers_{owner}_{repo}_{days}d.json` | JSON | 订阅用户列表 |

### 编程语言统计输出

| 文件名 | 格式 | 说明 |
|--------|------|------|
| `languages_{owner}_{repo}.json` | JSON | 编程语言占比 |
...
result = languages.run()

# 仓库综合报告（自动检测并采集所有模块数据）
...
│   ├── subscribers.py          # 订阅用户统计模块
│   ├── languages.py            # 编程语言统计模块
│   ├── report.py               # 仓库综合报告模块
│   └── utils.py                # 公共工具
├── config/                     # 配置文件目录
...
| `repo_stats.py` | `GitCodeRepoStats` 类，仓库综合统计 |
...
# 运行指定的测试类
pytest tests/test_community.py::TestGitCodeCommunityStats
```
